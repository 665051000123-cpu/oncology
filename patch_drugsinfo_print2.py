...
print_function = """    const printAllDrugs = async () => {
        ...
        if (currentUser?.a4_printer) {
            if (showNotification) showNotification('กำลังส่งข้อมูลไปยังเครื่องพิมพ์เอกสาร...', 'info');
            try {
                const res = await axios.post(`${API_BASE}/print`, {
                    html: htmlContent,
                    printerName: currentUser.a4_printer,
                    isA4: true
                });
                if (res.data.success) {
                    if (showNotification) showNotification(`พิมพ์รายการยาไปที่ ${currentUser.a4_printer} สำเร็จ`, 'success');
                    return;
                }
            } catch (err) {
...
content = content.replace('onClick={() => window.print()}', 'onClick={printAllDrugs}')

with io.open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Updated print function in DrugsInfo.jsx")
