...
function App() {
    ...
    const [adminRows, setAdminRows] = useState([
        { id: Date.now(), route: '', solvent: '', startDate: '', endDate: '', rate: '', order: 1, skipped: false }
    ]);
    ...
    const getLabRecommendations = () => {
        ...
        if (calcMode === 'single') {
            ...
            if (selectedRegimen === 'cv') {
                activeDrugs.push('carboplatin', 'vincristine');
            } else if (selectedRegimen === 'bc') {
                activeDrugs.push('bleomycin', 'carboplatin');
            }
        }
                        ...
                        <div className="md:col-span-7 premium-card p-6 md:p-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <input 
                                        type="text" 
                                        placeholder={prevStats.ward ? `หอผู้ป่วยคนก่อน: ${prevStats.ward}` : "หอผู้ป่วย (Ward) - เช่น Ward 5A, ICU (เว้นว่างหากไม่มี)"} 
                                        className="form-control" 
                                        value={patient.ward || ''}
                                    ...
                                    <input 
                                        type="text" 
                                        placeholder={prevStats.doctor ? `แพทย์คนก่อน: ${prevStats.doctor}` : "แพทย์ผู้รักษารอบนี้ (Doctor)"} 
                                        className="form-control" 
                                ...
                                <table className="w-full text-left text-sm md:text-base print-table">
                                    <thead className="sticky top-0 z-10" style={{ backgroundColor: 'var(--table-header-bg)' }}>
                                        <tr className="bg-sky-600/10 text-slate-400 border-b border-slate-700/20">
                                            ...
                                            {renderTableHeader('H.N.', 'w-[8%] whitespace-nowrap')}
                                            {renderTableHeader('ชื่อผู้ป่วย', 'w-[15%] whitespace-nowrap')}
                                            {renderTableHeader('หอผู้ป่วย', 'w-[8%] whitespace-nowrap')}
                                            {renderTableHeader('ประวัติแพ้ยา', 'w-[10%] whitespace-nowrap')}
                                            {renderTableHeader('เพศ', 'w-[6%] whitespace-nowrap', 'justify-center')}
                                            {renderTableHeader('อายุ', 'w-[6%] whitespace-nowrap', 'justify-center')}
                                            {renderTableHeader('BSA', 'w-[6%] whitespace-nowrap', 'justify-center')}
                                            {renderTableHeader('สูตรการคำนวณ', 'w-[14%] whitespace-nowrap')}
                                            {renderTableHeader('ยาที่ใช้', 'w-[10%] whitespace-nowrap')}
                                            {renderTableHeader('Dose', 'w-[10%] whitespace-nowrap', 'justify-end')}
                                            {renderTableHeader('แพทย์ผู้สั่ง', 'w-[10%] whitespace-nowrap')}
                                            {renderTableHeader('ผู้บันทึก', 'w-[9%] whitespace-nowrap', 'justify-center')}
                                            ...
                                            filteredLogs.map(log => (
                                                <tr key={log.id} className="border-b border-slate-700/10 hover:bg-sky-600/5 transition-colors">
                                                    <td className="p-4 font-mono opacity-70 whitespace-nowrap">{log.timestamp}</td>
                                                     <td 
                                                         onClick={() => setSearchQuery(log.hn)}
                                                         className="p-4 font-bold whitespace-nowrap text-sky-500 hover:text-sky-400 hover:underline cursor-pointer"
                                                         title={`กรองเฉพาะประวัติของ H.N. ${log.hn}`}
                                                     >
                                                         {log.hn}
                                                     </td>
                    ...
                    <div className="animate-row-in space-y-6">
                        ...
                        <div className="w-full premium-card p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
                            <div>
                                ...
                                <p className="text-xs text-sky-400 dark:text-sky-300 font-bold mt-1.5 flex flex-wrap gap-x-4">
                                    <span>🩺 แพทย์คนก่อน: <span className="text-slate-500 dark:text-slate-300 font-medium">{getPreviousDoctor()}</span></span>
                                    <span>🩺 แพทย์รอบนี้: <span className="text-sky-700 dark:text-sky-200 font-extrabold">{patient.doctor || 'ไม่ระบุ'}</span></span>
                                </p>
                                {patient.allergies && (
                                            ...
                                            {calcMode === 'single' ? (
                                                ...
                                                <div className="p-5 rounded-2xl border-2 border-indigo-400 dark:border-indigo-500/50 bg-indigo-50 dark:bg-indigo-950/40 transition-all shadow-md">
                                                    ...
                                                    <div className="mt-3 p-3 rounded-lg bg-white/60 dark:bg-black/30 border border-slate-200 dark:border-white/5">
                                                        <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 leading-relaxed">
                                                            {selectedRegimen === 'cv'
                                                                ? 'Carboplatin คำนวณตาม Calvert Formula (Target AUC × (CrCl + 25)) และ Vincristine คำนวณตามพื้นที่ผิว (BSA × 1.4 mg/m² พร้อม Dose Cap 2.0 mg)'
                                                                : 'Bleomycin ถูกคำนวณตามขนาดยาคงที่ (Fixed Dose 30 units) และ Carboplatin คำนวณตาม Calvert Formula (Target AUC × (CrCl + 25))'}
                                                        </p>
                                                    </div>
                                                </div>
                                            )}
                                ...
                                <div className="premium-card p-6">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                        ...
                                        <div className="p-4 rounded-xl border border-slate-700/20 bg-slate-800/10 space-y-4">
                                            <h3 className="text-xs font-black text-sky-500 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700/20 pb-2">
                                                ผลเลือดทางโลหิตวิทยา (Hematology)
                                            </h3>
                                        ...
                                        <div className="p-4 rounded-xl border border-slate-700/20 bg-slate-800/10 space-y-4">
                                            <h3 className="text-xs font-black text-amber-500 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700/20 pb-2">
                                        ...
                                        <div className="p-4 rounded-xl border border-slate-700/20 bg-slate-800/10 space-y-4">
                                            <h3 className="text-xs font-black text-emerald-500 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700/20 pb-2">
                                                การทำงานของไต (Renal - CrCl)
                                            </h3>
                                            <div className="space-y-3">
                                                <div className="grid grid-cols-2 gap-2">
                                                    <button
                                ...
                                <div className="premium-card p-6 mt-6">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <button
                                            ...
                                            className="btn btn-primary text-xs flex items-center gap-1 py-1.5 px-3 rounded-xl border border-sky-600 bg-sky-600/10 hover:bg-sky-600 text-white transition-all cursor-pointer font-bold"
                                        >
                                            + เพิ่มแถว
                                        </button>
                                    </div>
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-3 py-2.5">
                                                            <div className="flex gap-2 items-center">
                                                                <select
                                                                    value={
                                                                        ['', 'nss 50 ml', 'nss 100 ml', 'nss 250 ml', 'nss 500 ml', '5% d/w 50 ml', '5% d/w 100 ml', '5% d/w 250 ml', '5% d/w 500 ml'].includes((row.solvent || '').toLowerCase().trim())
                                                                            ? (row.solvent || '')
                                                                            : 'custom'
                                                                    }
                                                                    onChange={e => {
                                                                        const val = e.target.value;
                                                                        setAdminRows(prev => prev.map((r, i) => i === idx ? { ...r, solvent: val === 'custom' ? 'ระบุเอง' : val } : r));
                                                                    }}
                                                                    className="form-control py-1.5 px-3 text-xs rounded-lg font-bold min-w-[150px]"
                                                                >
                                                                    <option value="">-- เลือกตัวทำละลาย --</option>
                                                                    <option value="NSS 50 mL">NSS 50 mL</option>
                                                                    <option value="NSS 100 mL">NSS 100 mL</option>
                                                                    <option value="NSS 250 mL">NSS 250 mL</option>
                                                                    <option value="NSS 500 mL">NSS 500 mL</option>
                                                                    <option value="5% D/W 50 mL">5% D/W 50 mL</option>
                                                                    <option value="5% D/W 100 mL">5% D/W 100 mL</option>
                                                                    <option value="5% D/W 250 mL">5% D/W 250 mL</option>
                                                                    <option value="5% D/W 500 mL">5% D/W 500 mL</option>
                                                                    <option value="custom">ระบุเอง (Custom)...</option>
                                                                </select>

                                                                {(!['', 'nss 50 ml', 'nss 100 ml', 'nss 250 ml', 'nss 500 ml', '5% d/w 50 ml', '5% d/w 100 ml', '5% d/w 250 ml', '5% d/w 500 ml'].includes((row.solvent || '').toLowerCase().trim())) && (
                                                                    <input
                                                                        type="text"
                                                                        placeholder="ระบุตัวทำละลาย..."
                                                                        value={row.solvent === 'ระบุเอง' ? '' : row.solvent}
                                                                        onChange={e => setAdminRows(prev => prev.map((r, i) => i === idx ? { ...r, solvent: e.target.value } : r))}
                                                                        className="form-control py-1.5 px-3 text-xs rounded-lg font-bold min-w-[150px] animate-pop"
                                                                    />
                                                                )}
                                                            </div>
                                                        </td>
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    if (adminRows.length === 1) {
                                                                        ...
                                                                        setAdminRows(prev => prev.filter((_, i) => i !== idx));
                                                                    }
                                                                }}
                                                                ...
                                                                title={adminRows.length === 1 ? (row.skipped ? 'ยกเลิกการงดรับยา' : 'ทำเครื่องหมายงดรับยา') : 'ลบแถว'}
                                                            >
                                                                <span className="text-base font-black leading-none">{row.skipped ? '✓' : '−'}</span>
                                                            </button>
                                                        </td>
                                                    </tr>
                                                ))}
